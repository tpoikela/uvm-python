...
from uvm.reg.uvm_reg_model import UVM_NO_HIER
...
class UVMRegSingleBitBashSeq(UVMRegSequence):  # (uvm_sequence #(uvm_reg_item))
    ...
    def body(self):
        ...
        mode = []  # string [`UVM_REG_DATA_WIDTH]
        maps = []  # uvm_reg_map [$]
        dc_mask = 0x0
        reset_val = 0x0
        n_bits = 0
    ...
    #@cocotb.coroutine
    #   def bash_kth_bit(self,uvm_reg         rg,
    #                     int             k,
    #                     string          mode,
    #                     uvm_reg_map     map,
    ...
    #         bit_val = val[k]
    #
    #         rg.write(status, val, UVM_FRONTDOOR, map, self)
    #         if (status != UVM_IS_OK):
    #            `uvm_error("UVMRegBitBashSeq", sv.sformatf("Status was %s when writing to register \"%s\" through map \"%s\".",
    #                                        status.name(), rg.get_full_name(), map.get_full_name()))
    #         end
    #
    #         exp = rg.get() & ~dc_mask
    #         rg.read(status, val, UVM_FRONTDOOR, map, self)
    #         if (status != UVM_IS_OK):
    #            `uvm_error("UVMRegBitBashSeq", sv.sformatf("Status was %s when reading register \"%s\" through map \"%s\".",
    #                                        status.name(), rg.get_full_name(), map.get_full_name()))
    #         end
    #
    #         val &= ~dc_mask
    #         if (val != exp):
    #            `uvm_error("UVMRegBitBashSeq", sv.sformatf("Writing a %b in bit #%0d of register \"%s\" with initial value 'h%h yielded 'h%h instead of 'h%h",
