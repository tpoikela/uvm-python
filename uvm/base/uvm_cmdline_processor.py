...
class UVMCmdlineProcessor(UVMReportObject):
    ...
    def get_arg_values (self, match, values):
        ...
        for i in range(0, len(self.m_argv)):
            if len(self.m_argv[i]) >= chars:
                if self.m_argv[i][0:chars-1] == match:
                    val = self.m_argv[i][chars:len(self.m_argv[i]) - 1]
                    values.append(val)
        return len(values)
    ...
    def __init__(self, name = ""):
        ...
        for name in cocotb.plusargs:
            ...
            if s!="":
                if not name in self.m_plus_arg_map:
    ...
    def m_convert_verb(self, verb_str):
        ...
        if verb_srt == "MEDIUM":
            return UVM_MEDIUM
        if verb_srt == "UVM_MEDIUM":
            return UVM_MEDIUM
        if verb_srt == "HIGH":
            return UVM_HIGH
        if verb_srt == "UVM_HIGH":
            return UVM_HIGH
        if verb_srt == "FULL":
            return UVM_FULL
        if verb_srt == "UVM_FULL":
            return UVM_FULL
        if verb_srt == "DEBUG":
            return UVM_DEBUG
        if verb_srt == "UVM_DEBUG":
            return UVM_DEBUG
        return 0
